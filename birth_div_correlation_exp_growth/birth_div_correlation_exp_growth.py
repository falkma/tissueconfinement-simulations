...
#statistics of a G1-sizer model at varying (exponential) growth rates
#code for SI Fig 8B
folder = 'birth_div_correlation_exp_growth'
...
#set initial (multiplicative) growth of cells to provide baseline
initial_growth = .5*1.

#set growth rates for parameter sweep
growth_rates = .5*np.array([1.,.9*3./4.,.9*1./4.,1./20.])
    ...
    for _ in np.arange(replicates):
        ...
        for j in np.arange(num_steps):
            ...
            if j > 10*unit_time:
                ...
                a.append(a[-1]*(1+area_rate*dt))

            else:

                #initially we equilibrate cells via a high initial growth rate
                a.append(a[-1]*(1+initial_area_rate*dt))
